"""Benchmark a trained T5-small BitFit adapter on WikiSQL."""
...
def main() -> None:
    ...
    print(f"Saved BitFit benchmark index to {output_path}")
...
def benchmark_adapter(
    ...
    print(f"Benchmarking {model_spec.name} BitFit adapter on {len(examples)} WikiSQL examples")
    load_started_at = time.perf_counter()
    tokenizer = AutoTokenizer.from_pretrained(config["model"]["adapter_path"])
    model = load_bitfit_model(config)
    model.eval()
...
def load_bitfit_model(config: dict[str, Any]):
    model = AutoModelForSeq2SeqLM.from_pretrained(config["model"]["name"])
    if torch.cuda.is_available():
        model.to("cuda")
    bias_state = torch.load(Path(config["model"]["adapter_path"]) / "bitfit_biases.pt", map_location=model.device)
...
def build_benchmark_metadata(config: dict[str, Any], sample_size: int) -> dict[str, Any]:
    metadata = build_qlora_metadata(config, sample_size)
    metadata["mode"] = "bitfit"
    metadata["runner"] = "peft_lab.benchmark_t5_bitfit"
    return metadata


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser()
    parser.add_argument("--config", required=True, help="Path to BitFit benchmark YAML.")
