"""Benchmark a trained T5-small QLoRA adapter on WikiSQL."""
...
def main() -> None:
    ...
    print(f"Saved QLoRA benchmark index to {output_path}")
...
def benchmark_adapter(
    ...
    print(f"Benchmarking {model_spec.name} QLoRA adapter on {len(examples)} WikiSQL examples")
...
def load_adapter_model(config: dict[str, Any]):
    if not torch.cuda.is_available():
        raise RuntimeError("QLoRA adapter benchmarking requires CUDA.")
...
def build_benchmark_metadata(config: dict[str, Any], sample_size: int) -> dict[str, Any]:
    ...
    return {
        "task": "NL-to-SQL",
        "mode": "qlora",
        "runner": "peft_lab.benchmark_t5_qlora",
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--config", required=True, help="Path to QLoRA benchmark YAML.")
    parser.add_argument(
        "--max-examples",
        type=int,
        default=None,
