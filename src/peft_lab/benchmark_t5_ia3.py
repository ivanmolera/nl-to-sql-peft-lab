"""Benchmark a trained T5-small IA3 adapter on WikiSQL."""
...
def main() -> None:
    ...
    model_path = output_dir / "ia3_wikisql_t5-small.json"
    model_path.write_text(json.dumps(result, indent=2), encoding="utf-8")
    print(f"Saved IA3 model benchmark to {model_path}")
    ...
    print(f"Saved IA3 benchmark index to {output_path}")
...
def benchmark_adapter(
    ...
    print(f"Benchmarking {model_spec.name} IA3 adapter on {len(examples)} WikiSQL examples")
...
def build_benchmark_metadata(config: dict[str, Any], sample_size: int) -> dict[str, Any]:
    ...
    return {
        "task": "NL-to-SQL",
        "mode": "ia3",
        "runner": "peft_lab.benchmark_t5_ia3",
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--config", required=True, help="Path to IA3 benchmark YAML.")
