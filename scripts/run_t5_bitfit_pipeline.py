"""Run T5-small BitFit training, benchmark it, and optionally upload artifacts."""
...
DEFAULT_TRAIN_CONFIG = "configs/t5_small_wikisql_bitfit_full.yaml"
DEFAULT_BENCHMARK_CONFIG = "configs/t5_small_wikisql_bitfit_benchmark.yaml"
ARTIFACT_PATHS = [
    Path("outputs/t5-small-wikisql-bitfit"),
    Path("benchmark_results/bitfit"),
]
...
def main() -> None:
    ...
    run(["python3", "-m", "peft_lab.train_t5_bitfit", "--config", train_config])
    run(["python3", "-m", "peft_lab.benchmark_t5_bitfit", "--config", benchmark_config])
